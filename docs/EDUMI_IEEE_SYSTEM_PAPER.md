...
**Index Terms** — WebRTC, Selective Forwarding Unit, Face Recognition, Passive Liveness Detection, Django Channels, Redis Pub/Sub, Automated Attendance.
...
![Fig. 1. Four-Layer Decoupled System Architecture of Edumi2](../paper_assets/screenshots/fig1_system_architecture.png)
...
![Fig. 2. Meeting Join and Authentication Sequence Diagram](../paper_assets/screenshots/fig2_meeting_auth_sequence.png)
...
The `face_recognition` library with the `large` (68-point landmark) model extracts a 128-dimensional embedding. An OpenCV Haar cascade fallback (frontal face detection with equalized histogram and Sobel edge magnitude features) is engaged if the primary dlib library is unavailable.
...
![Fig. 3. Django Channels Dual-Group Signaling Architecture](../paper_assets/screenshots/fig3_signaling_architecture.png)
...
The full system is containerized using Docker Compose with seven service definitions. The `web` container serves all HTTP and WebSocket traffic through Daphne; no standalone `pages` service is defined:
...
services:
  ...
  livekit:         # livekit/livekit-server:v1.5.2
                   #   Ports: 7880 (HTTP/WS), 7881 (TCP RTC), 50100-50120/udp (RTC)
                   #   Config: livekit.yaml
  ...
  nginx:           # nginx:latest (port 80)
                   #   Proxies / → Daphne, /api/camera/ → CameraService,
                   #   /livekit-proxy/ → LiveKit, serves /static and /media volumes
```
