...
---
*Generated by Antigravity — Senior System Architect*

---

## 9. Major Modules and Classes
...
## 13. Comprehensive Database Schema
...
## 15. Internal API & Signaling Reference
...
To maintain the system's edge as an AI-powered educational tool, the following enhancements are planned:

*   **GPU Offloading**: Migrating the `FaceService` from CPU-bound Dlib to GPU-accelerated **TensorRT** models to support 100+ concurrent student streams on a single node.
*   **Edge Analytics**: Implementing **WebAssembly (WASM)** face detection in the browser to reduce the frequency of full-frame uploads, saving significant server bandwidth.
*   **Predictive Sleep Mode**: Using ML to predict classroom idle times and proactively spin down camera microservices before the "Idle Shutdown" timer hits.
*   **Advanced Emotion Depth**: Moving beyond simple liveness to "Cognitive Load" detection—analyzing eye-strain and blinking patterns to alert teachers when students are overwhelmed.
...
A new developer should use these methods to verify system integrity before pushing code:

*   **Manual AI Verification**: 
    1.  Go to `Attendance > Register Face`.
    2.  Check the `face_match_confidence` in the logs after registration.
    3.  Verify that the `embedding_checksum` is generated in the database.
*   **Signaling Stress Test**: Open the "Meeting Room" in 5+ browser tabs simultaneously to verify that the **Redis Channel Layer** can handle high-concurrency chat and signaling without dropping messages.
*   **Camera Microservice Test**: Access `http://localhost:8001/camera-service/stream/<ID>/` directly to ensure the FFmpeg remuxing pipeline starts correctly.
*   **Security Audit**: Verify that no raw `.jpg` or `.png` files are stored in `face_photos/` after a face reset request is processed.

---

## 18. Frontend State & UI Logic Deep-Dive
...
## 19. Onboarding: The First 30 Minutes
...
## 20. Contribution Best Practices
...
---
*Documentation Final Version — Senior System Architect*
